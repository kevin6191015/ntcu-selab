...
var Question = function(question){
 ...
 Question.getAllquestion = (result)=>{
    dbConn.query('SELECT * FROM question_bank2', (err,res)=>{
        if(err){
...
Question.createQuestion = (questionReqData, result) =>{
    ...
    console.log(changequestion);    
      dbConn.query('SELECT num FROM question_bank2 WHERE num = (SELECT MAX(num) FROM question_bank2)', (err,res)=>{
        var max_nums = JSON.parse(JSON.stringify(res));
        var max_num = max_nums[0].num;
        max_num = max_num+1;
        changequestion.num = max_num;
        max_num = padLeft(max_num,4);
        changequestion.id = 'b' + max_num;
        console.log(changequestion);
        //changequestion = JSON.stringify(changequestion);
        //console.log(changequestion)
        
        dbConn.query('INSERT INTO question_bank2 SET ? ',changequestion, (err, res)=>{
           if(err){
...
Question.deleteQuestion = (id, result)=>{
	dbConn.query("DELETE FROM question_bank2 WHERE id=?",id,(err,res)=>{
		if(err){
...
Question.getQuestionByTeacher = (teacher, result)=>{
    dbConn.query('SELECT * FROM question_bank2 WHERE teacher=?', teacher, (err,res)=>{
        if(err)
            result(err);
        else
            result(null,res);
