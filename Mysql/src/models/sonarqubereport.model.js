...
SQreport.getpersonalreport = (project_name, result) =>{
    dbConn.query('SELECT compile_result,source_code,report_suggestion,bugs,vulnerabilities,code_smells,submit_times FROM Sonarqube_reports WHERE project_name = ? ORDER BY submit_times DESC', project_name, (err,res)=>{
        if(err)
            result(err);
        else    
            result(res);         
